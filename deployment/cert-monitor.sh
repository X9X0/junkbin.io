...
${RENEW_HINT}" \
            "CRITICAL: cannot read cert for ${host}"
        continue
    fi
...
${RENEW_HINT}" \
            "CRITICAL: cert for ${host} expired ${ELAPSED} ago"
...
${RENEW_HINT}" \
            "CRITICAL: cert for ${host} expires in ${DAYS} days"
...
${RENEW_HINT}" \
            "WARNING: cert for ${host} expires in ${DAYS} days"
    fi
done

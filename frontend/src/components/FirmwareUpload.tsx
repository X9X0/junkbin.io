...
import { Upload, X, Loader2, CheckCircle, AlertCircle, Cpu, Clock, TrendingUp } from 'lucide-react';
...
const ALLOWED_EXTENSIONS = ['bin', 'hex', 'rom', 'img', 'dump', 'fw', 'elf', 'zip'];

export default function FirmwareUpload({ productId, onSuccess }: FirmwareUploadProps) {
  ...
  const SOURCE_TYPES = [
    { value: 'dumped', label: t('firmware.upload.source_dumped') },
    { value: 'community', label: t('firmware.upload.source_community') },
    { value: 'official', label: t('firmware.upload.source_official') },
    { value: 'reverse_engineered', label: t('firmware.upload.source_reverse') },
    { value: 'leaked', label: t('firmware.upload.source_leaked') },
    { value: 'unknown', label: t('firmware.upload.source_unknown') },
  ];
  ...
  const getUploadErrorMessage = (error: unknown): string => {
    if (axios.isAxiosError(error)) {
      ...
      if (error.config?.url?.includes('/auth/token/refresh/')) {
        return t('firmware.upload.error_session_expired');
      }
      if (status === 429) {
        return t('firmware.upload.error_rate_limited');
      }
    ...
    return t('firmware.upload.error');
  };
  ...
  const uploadMutation = useMutation({
    mutationFn: async (previewFile: PreviewFile) => {
      ...
      if (previewFile.chipArchitecture) {
        formData.append('chip_architecture', previewFile.chipArchitecture);
      }
      return products.uploadFirmware(productId, formData);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['product', productId] });
      queryClient.invalidateQueries({ queryKey: ['product', productId, 'firmware'] });
      setFile(null);
      onSuccess?.();
    },
  });
  ...
  const handleFile = (newFile: File | null) => {
    ...
    // Check file size (max 50MB for firmware)
      ...
      {!file && (
        ...
        >
          <input
            ref={fileInputRef}
            type="file"
            accept=".bin,.hex,.rom,.img,.dump,.fw,.elf,.zip"
          ...
          <p className="text-gray-400 mb-1">
            {t('firmware.upload.drop_hint')}
          </p>
          <p className="text-xs text-gray-600 font-mono">
            {t('firmware.upload.drop_accept')}
          </p>
        </div>
      )}
        ...
        <div className="card-cyber p-4 space-y-4">
            ...
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 bg-cyber-green/20 border border-cyber-green/30 flex items-center justify-center">
                <span className="text-cyber-green font-mono text-xs font-bold">
                  {(file.file.name.split('.').pop() || 'BIN').toUpperCase()}
                </span>
              </div>
              <div>
          ...
          <div>
            <label className="block text-xs font-mono text-gray-500 mb-1">
              {t('firmware.upload.title_label')} <span className="text-cyber-pink">*</span>
            </label>
            <input
              type="text"
              value={file.title}
              onChange={(e) => updateFile({ title: e.target.value })}
              placeholder={t('firmware.upload.title_placeholder')}
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-mono text-gray-500 mb-1">
                {t('firmware.upload.source_label')}
              </label>
              <select
            ...
            <div>
              <label className="block text-xs font-mono text-gray-500 mb-1">
                {t('firmware.upload.version_label')}
              </label>
              <input
                type="text"
                value={file.version}
                onChange={(e) => updateFile({ version: e.target.value })}
                placeholder={t('firmware.upload.version_placeholder')}
                className="input-cyber w-full"
              />
            </div>
          </div>

          {/* Chip/Architecture and Source URL */}
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-mono text-gray-500 mb-1">
                {t('firmware.upload.chip_architecture_label')}
              </label>
              <input
                type="text"
                value={file.chipArchitecture}
                onChange={(e) => updateFile({ chipArchitecture: e.target.value })}
                placeholder={t('firmware.upload.chip_architecture_placeholder')}
                className="input-cyber w-full"
              />
            </div>
            <div>
              <label className="block text-xs font-mono text-gray-500 mb-1">
                {t('firmware.upload.source_url_label')}
              </label>
              <input
                type="url"
          ...
          <div>
            <label className="block text-xs font-mono text-gray-500 mb-1">
              {t('firmware.upload.description_label')}
            </label>
            <textarea
              value={file.description}
              onChange={(e) => updateFile({ description: e.target.value })}
              placeholder={t('firmware.upload.description_placeholder')}
              rows={3}
            ...
            >
              {uploadMutation.isPending ? (
                <>
                  ...
                  {t('firmware.upload.submitting')}
                </>
              ) : (
                <>
                  <Upload className="h-4 w-4" />
                  {t('firmware.upload.submit')}
                </>
              )}
            </button>
          </div>
